...
def get_answer_kb(chat_id):
    ...
    return answer_kb
def sheet_expired(chat_id):
    sh = gc.open_by_url(table_url)
    worksheet = sh.worksheet(title=chat_id)
    if worksheet.cell(row=2, col=4).value == 0:
        return True
    make_conclusion(chat_id)
...
def make_conclusion(chat_id):
    sh = gc.open_by_url()
    worksheet = sh.worksheet(chat_id)
    users_num = len(worksheet.col_values(2)-3)
    for i in range(users_num):
        row = worksheet.row_values(i+4)
        num_questions = get_user_current_question(chat_id, user_id=row[1])
        if num_questions == None:
            Y = sum([i for i in row[2:12] if i])/10
        elif num_questions == 1:
            Y = 0
        else:
            Y = sum([i for i in row[2:12] if i])/(num_questions-1)
        worksheet.update_cell(row = i+4,col=13, value=Y)
        if Y >= 3:
            udo = 'высокая удовлетворённость'
        elif Y >= 2 and Y < 3:
            udo = 'средняя удовлетворённость'
        else:
            udo = 'низкая удовлетворённость'
        worksheet.update_cell(row = i+4,col= 14, value=udo)
    change_poll_status(chat_id,0)
...
scheduler.add_job(check_full_done_sheets, 'interval',hours = 1)
